...
class WireNetwork:
        ...
        for i in update:
            match self.tilemap[i][0]:
                case TileTypes.WIRE_B | TileTypes.WIRE_G:
                    ...
                    for offset in OFFSETS:
                        try: 
                            if (self.tilemap[i[0] + offset[0], i[1] + offset[1]][1] and self.tilemap[i[0] + offset[0], i[1] + offset[1]][0] not in self.WIRE_TYPES): 
                                
                                state = True
                        except KeyError: pass
                    
                    conected_wires, state = self.check_wire_conections(i, self.tilemap[i][0])
                    for wire in conected_wires:
    ...
    def check_wire_conections(self, wire_pos, type) -> list:
        next_wires = [wire_pos]
        checked_wires = []
        new_state = False
        while True:
            ...
            for wire in wires:
                for offset in OFFSETS:
                    if (wire[0] + offset[0], wire[1] + offset[1]) in self.tilemap:
                        ...
                        if tile_pos not in checked_wires: 
                            if self.tilemap[tile_pos][0] == type:
                                next_wires.append(tile_pos)
                                checked_wires.append(tile_pos)

                            elif self.tilemap[tile_pos][0] not in self.WIRE_TYPES:
                                if tile_pos not in self.next_update: 
                                    self.next_update.append(tile_pos)
                                if self.tilemap[tile_pos][1] == 1:
                                    new_state = True
            if next_wires == []:
                return checked_wires, new_state
